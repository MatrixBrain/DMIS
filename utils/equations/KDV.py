...
@problem_decorator
class KDV(ProblemDefine2d):

    def __init__(self, problem_conf, data_conf):
        super(KDV, self).__init__(problem_conf, data_conf)
    ...
    def pde_loss(self, pred, input_tensor):
        ...
        df_dxxx = fwd_gradients(df_dxx, input_tensor)[:, 1:2]

        pde_output = df_dt + pred * df_dx + 0.0025 * df_dxxx
        return pde_output
